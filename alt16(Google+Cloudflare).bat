...
--filter-tcp=443 --hostlist="%LISTS%list-google.txt" --hostlist="%LISTS%list-youtube.txt" --ip-id=zero --dpi-desync=chaos --dpi-desync-repeats=15 --dpi-desync-split-seqovl=681 --dpi-desync-split-pos=1 --dpi-desync-split-seqovl-pattern="%BIN%tls_clienthello_www_google_com.bin" --new ^
--filter-tcp=80 --hostlist="%LISTS%list-google.txt" --hostlist="%LISTS%list-youtube.txt" --dpi-desync=chaos --dpi-desync-repeats=15 --dpi-desync-split-seqovl=568 --dpi-desync-split-pos=1 --dpi-desync-split-seqovl-pattern="%BIN%tls_clienthello_www_google_com.bin" --new ^
--filter-udp=443 --hostlist="%LISTS%list-google.txt" --hostlist="%LISTS%list-youtube.txt" --hostlist-exclude="%LISTS%list-exclude.txt" --hostlist-exclude="%LISTS%list-exclude-user.txt" --ipset-exclude="%LISTS%ipset-exclude.txt" --ipset-exclude="%LISTS%ipset-exclude-user.txt" --dpi-desync=fake --dpi-desync-repeats=10 --dpi-desync-fake-quic="%BIN%quic_initial_www_google_com.bin" --dpi-desync-any-protocol --dpi-desync-cutoff=d3 --new ^
